function Hhat = copyAuthorsCode(sCall,fCall)
...
for jj=1:numSubj
    ...
    % sparsify
    MapC = MapC .* (MapC > epsilon*max(MapC(:)));
    ...
    [Mtx.H{jj},ErrH{jj},~,tvec] = LinearModel(MapC,corr_graphs,inds);
    [corCoef,Pval.pval{jj}] = corrcoef((Mtx.H{jj} .* inds), (corr_graphs .* inds));
...
for ii=1:nt
    ...
    [Err,pval{ii}] = corrcoef((Hhat.*inds), (corr_graphs.*inds));  % corrcoef(Hhat(inds),corr_graphs(inds)); % corrcoef(vectorize_matrix(Hhat.*inds), vectorize_matrix(corr_graphs.*inds));
    corCoef(ii) = Err(1,2);
end
