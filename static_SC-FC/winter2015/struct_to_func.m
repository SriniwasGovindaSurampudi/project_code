...
L = diag(sum(W_s,1))-W_s; %unNormalized Lap
...
%}
val=diag(val);
scale_v=[ 10^(-3) 10^(-2) 10^(-1) 0.5 1 2];
for i = 1:length(scale_v)
    
    H_s = vec * diag(exp(-val*scale_v(i))) * vec';                          %Heat Kernal

    %Normalizing in the range [0 1]
    H_s2{i} = H_s; %(H_s - min(H_s(:)))/(max(H_s(:)) - min(H_s(:))); %Normalized Heat Kernal
